...
# Modify these values according to your project's needs.
PROJECT_NAME="KSU-Kinesis"
KERNEL_SOURCE="https://github.com/AzyrRuthless/kernel_xiaomi_sm6250"
KERNEL_BRANCH="xx" # Branch of the kernel source
DEVICE_CODENAME="miatoll" # Your device's codename
DEFCONFIG="vendor/xiaomi/miatoll_defconfig" # Path to your defconfig

# --- Telegram Notification Configuration (Optional) ---
# Set TELEGRAM_CHAT_ID and TELEGRAM_BOT_TOKEN in Gitpod Environment Variables.
TELEGRAM_CHAT_ID="${TELEGRAM_CHAT_ID}"
TELEGRAM_BOT_TOKEN="${TELEGRAM_BOT_TOKEN}"
USE_TELEGRAM_NOTIF=false
...
# --- Function to send Telegram text messages ---
tg() {
...
sudo apt-get update
sudo apt-get install -y git bison flex libxml2 libssl-dev bc zip default-jre python3 python3-pip wget curl cpio bash ccache jq
...
KERNEL_DIR="/workspace/$PROJECT_NAME"
...
if ! git clone "$KERNEL_SOURCE" -b "$KERNEL_BRANCH" "$KERNEL_DIR" --depth=1; then
  handle_error "Failed to clone kernel source"
fi
cd "$KERNEL_DIR" || handle_error "Failed to enter kernel directory"
...
cd "$KERNELSU_DIR"
git stash >/dev/null && log "➖ Stashed current changes."
git checkout next >/dev/null && log "➖ Switched to 'next' branch."
git pull >/dev/null && log "🔄 KernelSU-Next repository updated."
cd "$KERNEL_DIR"
...
IFS=- read -r KERNEL_VARIANT KERNEL_CODENAME_DEF KERNEL_RELEASE_VERSION <<< "$RELEASE_VERSION" || true
log "ℹ️ Kernel Variant: $KERNEL_VARIANT"
log "ℹ️ Kernel Codename: $DEVICE_CODENAME"
log "ℹ️ Release Version: $KERNEL_RELEASE_VERSION"
...
# --- Make defconfig ---
log "⚙️ Generating defconfig using: $DEFCONFIG"
...
if [ ${PIPESTATUS[0]} -ne 0 ]; then
  handle_error "Compilation failed. Check build.log for details."
...
if ! git clone -q -b Ivory https://github.com/AzyrRuthless/AnyKernel3 "$KERNEL_DIR/anykernel"; then
  handle_error "Failed to clone AnyKernel3"
fi

# --- Copy build artifacts to AnyKernel3 directory ---
log "➡️ Copying Image.gz..."
cp "$KERNEL_DIR/out/arch/arm64/boot/Image.gz" "$KERNEL_DIR/anykernel"
log "➡️ Copying dtbo.img..."
cp "$KERNEL_DIR/out/arch/arm64/boot/dtbo.img" "$KERNEL_DIR/anykernel"
log "📁 Creating dtb directory in AnyKernel3..."
mkdir -p "$KERNEL_DIR/anykernel/dtb"
log "➡️ Copying cust-atoll-ab.dtb..."
cp "$KERNEL_DIR/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$KERNEL_DIR/anykernel/dtb"

# --- Create ZIP archive ---
ZIP_NAME="${PROJECT_NAME}-${KERNEL_VARIANT}-${DEVICE_CODENAME}-${KERNEL_RELEASE_VERSION}-$(date '+%Y%m%d').zip"
log "🗜️ Creating ZIP archive: $ZIP_NAME"
cd "$KERNEL_DIR/anykernel" || handle_error "Failed to enter AnyKernel3 directory"
...
tg_doc "$KERNEL_DIR/$ZIP_NAME" "✅ Build finished after ${BUILD_DURATION_MINUTES} minutes ${BUILD_DURATION_SECONDS} seconds"
...
cp "$KERNEL_DIR/out/arch/arm64/boot/Image.gz" "$ARTIFACT_DIR/"
cp "$KERNEL_DIR/out/arch/arm64/boot/dtbo.img" "$ARTIFACT_DIR/"
cp "$KERNEL_DIR/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb" "$ARTIFACT_DIR/"
cp "$KERNEL_DIR/$ZIP_NAME" "$ARTIFACT_DIR/"
